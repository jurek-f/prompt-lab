...
export function RegressionPanel({ systemPrompt, responseModel, evalModel, tutorEnabled, pastSessions, ollamaResponseModel, ollamaEvalModel, ollamaBaseUrl, onTutorAnalysis }: Props) {
  ...
  } = useRegressionTests(systemPrompt, responseModel, evalModel, tutorEnabled, ollamaResponseModel, ollamaEvalModel, ollamaBaseUrl, onTutorAnalysis)
    ...
    reader.onload = ev => {
      try {
        ...
        for (const item of raw) {
          if (typeof item?.query === 'string' && typeof item?.targetAnswer === 'string') {
            addTestCase({
              label: typeof item.label === 'string' ? item.label : item.query.slice(0, 50),
              query: item.query,
              targetAnswer: item.targetAnswer,
              passThreshold: typeof item.passThreshold === 'number' ? item.passThreshold : undefined,
            })
            count++
          }
        }
    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 10 }}>
      ...
      {open && (
        <div style={{ borderTop: '1px solid var(--border)', padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <AddTestCaseForm onAdd={addTestCase} suites={suites} onLoadSuite={handleLoadSuite} />
...
function AddTestCaseForm({ onAdd, suites, onLoadSuite }: {
    ...
    return (
      <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
        ...
        {suites.length > 0 && (
          <select
            value={selectedSuite}
            onChange={e => { setSelectedSuite(e.target.value); onLoadSuite(e.target.value) }}
            style={{
              fontSize: '0.82rem',
              padding: '0.4rem 0.6rem',
...
const textareaStyle: React.CSSProperties = {
  ...
  fontFamily: 'var(--sans)', lineHeight: 1.5, boxSizing: 'border-box',
}
