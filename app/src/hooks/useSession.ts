...
import { pushSessionHistory } from '../lib/overhang'
...
interface WorkspaceOpts {
  ...
  onSessionPushed?: () => void
}

export function useSession({ onSessionPushed, ...ws }: WorkspaceOpts = {}) {
  ...
  }, [session, pastSessions, loading, responseModelId, evalModelId, workspaceId, ollamaResponseModel, ollamaEvalModel, ollamaBaseUrl])
  ...
  }, [session, pastSessions, tutorLoading, evalModelId, workspaceId, ollamaEvalModel, ollamaBaseUrl])
  ...
  const summarizeAndStartNew = useCallback(async () => {
    ...
    try {
      ...
      if (workspaceId) {
        pushSessionHistory(workspaceId, summary).then(() => onSessionPushed?.()).catch(() => {})
      }
  ...
  }, [session, pastSessions, evalModelId, workspaceId, ollamaEvalModel, ollamaBaseUrl])
